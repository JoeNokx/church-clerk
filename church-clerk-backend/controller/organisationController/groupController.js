...
const getOrganisationKPI = async (req, res) => {
  try {
    const orgQuery = { church: req.activeChurch._id };
    ...
    const [
      ...
      Group.countDocuments(orgQuery),
      Cell.countDocuments(orgQuery),
      Department.countDocuments(orgQuery),
      Ministry.countDocuments(orgQuery),
      Group.countDocuments({ ...orgQuery, createdAt: { $lt: startOfMonth } }),
      Cell.countDocuments({ ...orgQuery, createdAt: { $lt: startOfMonth } }),
      Department.countDocuments({ ...orgQuery, createdAt: { $lt: startOfMonth } }),
      Ministry.countDocuments({ ...orgQuery, createdAt: { $lt: startOfMonth } }),
    ]);

    const totalOrganisation = totalGroups + totalCells + totalDepartments + totalMinistries;
    const totalOrganisationPrev = totalGroupsPrev + totalCellsPrev + totalDepartmentsPrev + totalMinistriesPrev;
    ...
    const change = {
      ...
      totalMinistries: pctChange(totalMinistries, totalMinistriesPrev),
      totalOrganisation: pctChange(totalOrganisation, totalOrganisationPrev)
    };
    ...
    const diff = {
      ...
      totalMinistries: totalMinistries - totalMinistriesPrev,
      totalOrganisation: totalOrganisation - totalOrganisationPrev
    };

    return res.status(200).json({
      message: "Organisation KPI fetched successfully",
      summary: {
        totalGroups,
        totalCells,
        totalDepartments,
    ...
    return res.status(500).json({
      message: "Could not fetch OrganisationKPI",
      error: error.message,
    });
  }
};
...
export {createGroup, getAllGroups, getSingleGroup, updateGroup, deleteGroup, searchMembersToAddToGroup, addMemberToGroup, updateMemberRole, removeMemberFromGroup, getGroupMembers, addMeeting, updateMeeting, deleteMeeting, getMeetings, getOrganisationKPI }
