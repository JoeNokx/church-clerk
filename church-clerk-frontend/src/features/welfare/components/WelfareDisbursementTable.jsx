        ...
        <table className="min-w-full">
          <thead className="bg-slate-100">
            <tr className="text-left md:max-lg:text-sm font-semibold text-gray-500 text-xs">
              ...
              <th className="max-md:px-4 py-2 whitespace-nowrap px-4 md:px-6">Date Disbursed</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {rows.map((row, index) => (
              <tr key={row?._id ?? `row-${index}`} className="max-md:text-xs text-gray-700 text-sm">
                <td className="sticky left-0 z-10 bg-white max-md:px-4 py-1.5 text-gray-900 whitespace-nowrap px-4 md:px-6" title={row?.beneficiaryName || "-"}><span className="sm:hidden">{truncateMobileName(row?.beneficiaryName || "-")}</span><span className="hidden sm:inline">{truncateDesktopName(row?.beneficiaryName || "-")}</span></td>
                <td className="max-md:px-4 py-1.5 text-gray-900 whitespace-nowrap px-4 md:px-6" title={row?.category || "-"}><span className="sm:hidden">{truncateMobileName(row?.category || "-")}</span><span className="hidden sm:inline">{truncateDesktopName(row?.category || "-")}</span></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 overflow-y-auto">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-xl">
            <div className="flex items-start justify-between gap-4 border-b border-gray-200 px-4 md:px-6 py-4">
              <div>
                <div className="font-semibold text-gray-900 text-sm">{viewRow?.beneficiaryName || "Disbursement"}</div>
            ...
            <div className="p-4 md:p-6 space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><div className="font-semibold text-gray-500 text-xs">Date Disbursed</div><div className="mt-1 text-gray-900">{formatDate(viewRow?.date)}</div></div>
