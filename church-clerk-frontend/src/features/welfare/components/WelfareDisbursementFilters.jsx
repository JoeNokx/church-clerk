...
function WelfareDisbursementFilters() {
  ...
  const clearDates = async () => {
    ...
    await store?.fetchDisbursements?.({ dateFrom: "", dateTo: "", page: 1 });
    setDatePickerOpen(false);
  };
  ...
  const applyDates = async () => {
    ...
    if ((from && !to) || (!from && to)) {
      ...
      store?.setDisbursementFilters?.({ dateFrom: single, dateTo: single, page: 1 });
      await store?.fetchDisbursements?.({ dateFrom: single, dateTo: single, page: 1 });
      setDatePickerOpen(false);
      return;
    }
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
      <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-2">
        ...
        <input
          ...
          className="h-10 w-full sm:w-[240px] rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-700"
