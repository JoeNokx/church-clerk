              ...
              <table className="min-w-full">
                <thead className="bg-slate-100">
                  <tr className="text-left text-xs sm:max-lg:text-sm font-semibold text-gray-500">
                    <th className="sticky left-0 z-20 bg-slate-100 px-6 max-sm:px-4 py-2 whitespace-nowrap">Branch</th>
                    <th className="px-6 max-sm:px-4 py-2 whitespace-nowrap">Location</th>
                    <th className="px-6 max-sm:px-4 py-2 whitespace-nowrap">Pastor</th>
                    <th className="px-6 max-sm:px-4 py-2 whitespace-nowrap">Members</th>
                    <th className="px-6 max-sm:px-4 py-2 text-right whitespace-nowrap">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {branches.map((b, idx) => (
                    <tr key={b?._id ?? `b-${idx}`} className="text-sm max-sm:text-xs text-gray-700">
                      <td className="sticky left-0 z-10 bg-white px-6 max-sm:px-4 py-1.5 text-gray-900 whitespace-nowrap">{b?.name || "—"}</td>
                      <td className="px-6 max-sm:px-4 py-1.5 whitespace-nowrap">{`${b?.city || ""}${b?.region ? `, ${b.region}` : ""}`.trim() || "—"}</td>
                      <td className="px-6 max-sm:px-4 py-1.5 whitespace-nowrap">{b?.pastor || "—"}</td>
                      <td className="px-6 max-sm:px-4 py-1.5 text-blue-700 whitespace-nowrap">{Number(b?.memberCount || 0).toLocaleString()}</td>
