      ...
      <Card.Header
        ...
        actions={
          <>
            ...
            {canDelete && team?.canDelete !== false ? (
                      ...
                      {members.map((m, i) => {
                        ...
                        return (
                          <tr key={i} className="max-md:text-xs text-gray-700 text-sm">
                            <td className="max-md:px-4 py-1.5 text-gray-900 whitespace-nowrap px-4 md:px-6 sticky left-0 z-10 bg-white" title={name}>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {events.map((ev) => (
                        <tr key={ev._id} className="max-md:text-xs text-gray-700 text-sm">
                          <td className="max-md:px-4 py-1.5 text-gray-900 whitespace-nowrap px-4 md:px-6 sticky left-0 z-10 bg-white" title={ev.title}>
                            <div className="text-sm font-semibold text-gray-900">
...
export default function TeamsTab({ focusTeamId, setHeaderAction }) {
  ...
  useEffect(() => {
    ...
    setHeaderAction(
      <button onClick={() => guarded(() => { setEditingTeam(null); setFormMode("create"); setFormOpen(true); })} className="cck-allow-icons h-9 inline-flex items-center gap-2 rounded-lg bg-blue-700 px-4 text-sm font-semibold text-white hover:bg-blue-800 shrink-0">
  ...
  }, [canCreate, setHeaderAction, guarded]);
        ...
        {loading ? (
          ...
          <div className="p-4 md:p-6 lg:p-8">
            <EmptyState
              ...
              onAction={search ? () => handleSearchChange("") : (canCreate ? () => guarded(() => { setEditingTeam(null); setFormMode("create"); setFormOpen(true); }) : undefined)}
            />
          </div>
        ) : (
          <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 md:p-6 lg:p-8">
            {paginatedTeams.map((team) => (
              <TeamCard
                ...
                onEdit={(t) => guarded(() => { setEditingTeam(t); setFormMode("edit"); setFormOpen(true); })}
                onDelete={(t) => guarded(() => setDeleteTarget(t))}
