...
function ReportsAnalyticsPage() {
  ...
  const removeSaved = async (row) => {
    ...
    try {
      ...
      setSaved((prev) => prev.filter((r) => r._id !== row._id));
    } catch {
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="mt-6 rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
        <div className="flex items-start justify-between gap-3 border-b border-gray-200 bg-gray-50 px-4 py-2.5 md:px-6">
          <div>
          ...
          <div className="mt-2">
            {savedLoading ? (
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          {canExport ? (
                            <button
                              type="button"
                              title="Download PDF"
                              disabled={downloadingKey === `${row?._id}:pdf`}
                              onClick={() => downloadSaved(row, "pdf")}
                              className="cck-allow-icons h-8 w-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-50"
                            >
                              <DownloadIcon />
                            </button>
              ...
              <EmptyState
                ...
                description="Generate a report from any module card above and it will appear here."
              />
            )}
          </div>
        </div>
      ...
      <ReportGenerateModal
        ...
        onSaved={(doc) => {
          if (doc?._id) setSaved((prev) => [doc, ...prev]);
        }}
      />
