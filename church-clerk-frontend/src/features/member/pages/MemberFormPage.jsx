...
function MemberFormPageInner() {
  ...
  const hydrateFromMember = useCallback((m) => {
    ...
    setDateOfBirth(m?.dateOfBirth ? String(m.dateOfBirth).slice(0, 10) : "");

    setStreetAddress(m?.streetAddress || "");
    setCity(m?.city || "");
    setRegion(m?.region || "");
    setCountry(m?.country || "Ghana");
    setMaritalStatus(m?.maritalStatus || "");

    setChurchRole(m?.churchRole || "");
    setDateJoined(m?.dateJoined ? String(m.dateJoined).slice(0, 10) : "");
  ...
  const fetchMinistry = useCallback(async () => {
    ...
    try {
      ...
      setDepartments(
        Array.isArray(departmentsPayload?.departments)
  ...
  useEffect(() => {
    ...
    hydrateFromMember(prefill);
    if (prefill?._id) setPageMode("edit");

    navigate(location.pathname + location.search, { replace: true, state: null });
  }, [hydrateFromMember, location.pathname, location.search, location.state, navigate]);
