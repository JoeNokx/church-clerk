...
function SpecialFundFilters() {
  ...
  }, [datePickerOpen, draftFrom, draftTo, store]);
  ...
  const clearDates = async () => {
    ...
    await store?.fetchSpecialFunds({ dateFrom: "", dateTo: "", page: 1 });
    setDatePickerOpen(false);
  };
  ...
  const onDraftFromChange = (value) => {
    ...
    // If user changes From, and To is before it, reset To.
    if (draftTo && value && draftTo < value) {
      setDraftTo("");
    }
  };

  const onDraftToChange = async (value) => {
    setDraftTo(value);
  };
  ...
  const applyDates = async () => {
    ...
    if ((from && !to) || (!from && to)) {
      ...
      store?.setFilters({ dateFrom: single, dateTo: single, page: 1 });
      await store?.fetchSpecialFunds({ dateFrom: single, dateTo: single, page: 1 });
      setDatePickerOpen(false);
      return;
    }
