        ...
        <table className="min-w-full">
          <thead className="bg-slate-100">
            <tr className="text-left text-xs sm:max-lg:text-sm font-semibold text-gray-500">
              <th className="sticky left-0 z-20 bg-slate-100 px-6 max-sm:px-4 py-2 whitespace-nowrap">Giver Name</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {rows.map((fund, index) => (
              <tr key={fund?._id ?? `row-${index}`} className="text-sm max-sm:text-xs text-gray-700">
                <td className="sticky left-0 z-10 bg-white px-6 max-sm:px-4 py-1.5 text-gray-700 whitespace-nowrap">{fund?.giverName || "-"}</td>
                <td className="px-6 max-sm:px-4 py-1.5 text-gray-900 whitespace-nowrap">{fund?.category || "-"}</td>
                <td className="px-6 max-sm:px-4 py-1.5 text-blue-700 whitespace-nowrap">{formatMoney(fund?.totalAmount || 0, currency)}</td>
                <td className="px-6 max-sm:px-4 py-1.5 whitespace-nowrap">{formatDate(fund?.givingDate)}</td>
                <td className="px-6 max-sm:px-4 py-1.5 text-gray-600 max-w-[360px] break-words">{fund?.description || "-"}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-sm rounded-xl bg-white shadow-xl">
            <div className="border-b border-gray-200 px-5 py-4">
              <div className="text-sm font-semibold text-gray-900">Delete Fund</div>
            </div>
