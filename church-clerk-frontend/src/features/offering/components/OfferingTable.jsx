        ...
        <table className="min-w-full">
          <thead className="bg-slate-100">
            <tr className="text-left md:max-lg:text-sm font-semibold text-gray-500 text-xs">
              ...
              <th className="max-md:px-4 py-2 whitespace-nowrap px-4 md:px-6">Offering Type</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {rows.map((offering, index) => (
              <tr key={offering?._id ?? `row-${index}`} className="max-md:text-xs text-gray-700 text-sm">
                <td className="sticky left-0 z-10 bg-white max-md:px-4 py-1.5 text-gray-900 whitespace-nowrap px-4 md:px-6" title={offering?.serviceType || "-"}><span className="sm:hidden">{truncateMobileName(offering?.serviceType || "-")}</span><span className="hidden sm:inline">{truncateDesktopName(offering?.serviceType || "-")}</span></td>
                <td className="max-md:px-4 py-1.5 text-gray-700 whitespace-nowrap px-4 md:px-6" title={offering?.offeringType || "-"}><span className="sm:hidden">{truncateMobileName(offering?.offeringType || "-")}</span><span className="hidden sm:inline">{truncateDesktopName(offering?.offeringType || "-")}</span></td>
                <td className="max-md:px-4 py-1.5 text-blue-700 whitespace-nowrap px-4 md:px-6">{formatMoney(offering?.amount || 0, currency)}</td>
                <td className="max-md:px-4 py-1.5 whitespace-nowrap px-4 md:px-6">{formatDate(offering?.serviceDate)}</td>
                ...
                <td className="max-md:px-4 py-1.5 whitespace-nowrap px-4 md:px-6">
                  <TableKebabMenu items={[
                    { label: "View", onClick: () => { setViewRow(offering); setViewOpen(true); } },
                    canEdit && { label: "Edit", onClick: () => guarded(() => { if (!offering?._id) return; onEdit?.(offering); }) },
                    canDelete && { label: "Delete", onClick: () => guarded(() => { if (!offering?._id) return; openConfirmDelete(offering._id); }), danger: true }
                  ]} />
                </td>
              </tr>
            ))}
      ...
      {viewOpen && viewRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 overflow-y-auto">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-xl">
            ...
            <div className="p-4 md:p-6 grid grid-cols-2 gap-3 text-sm">
              <div><div className="font-semibold text-gray-500 text-xs">Service Type</div><div className="mt-1 text-gray-900">{viewRow?.serviceType || "-"}</div></div>
              <div><div className="font-semibold text-gray-500 text-xs">Offering Type</div><div className="mt-1 text-gray-900">{viewRow?.offeringType || "-"}</div></div>
              <div><div className="font-semibold text-gray-500 text-xs">Amount</div><div className="mt-1 text-blue-700 font-semibold">{formatMoney(viewRow?.amount || 0, currency)}</div></div>
              <div><div className="font-semibold text-gray-500 text-xs">Date</div><div className="mt-1 text-gray-900">{formatDate(viewRow?.serviceDate)}</div></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 overflow-y-auto">
          <div className="w-full max-w-sm max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-xl">
            <div className="border-b border-gray-200 px-4 md:px-5 lg:px-6 py-4">
              <div className="font-semibold text-gray-900 text-sm">Delete Offering</div>
            </div>
