...
function ChurchProjectDetailsPage() {
  ...
  const [viewContribRow, setViewContribRow] = useState(null);
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="mt-6 rounded-xl border border-gray-200 bg-white">
        ...
        {tab === "contributions" ? (
          <div>
            {contribLoading ? <div className="p-5 flex items-center justify-center"><Spinner className="text-gray-400" /></div> : null}
            {contribError ? (
              <div className="p-5">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {contribRows.map((row, idx) => (
                        <tr key={row?._id ?? `c-${idx}`} className="text-sm text-gray-700">
                          <td className="px-6 py-1.5 text-gray-900" title={row?.contributorName || ""}>
                            <span className="sm:hidden">{truncateMobileName(row?.contributorName)}</span>
                            <span className="hidden sm:inline">{truncateDesktopName(row?.contributorName)}</span>
                          </td>
                          ...
                          <td className="px-6 py-1.5" title={row?.createdBy?.fullName || ""}>
                            <span className="sm:hidden">{truncateMobileName(row?.createdBy?.fullName)}</span>
                            <span className="hidden sm:inline">{truncateDesktopName(row?.createdBy?.fullName)}</span>
                          </td>
                          <td className="px-6 py-1.5">
                            <div className="flex items-center justify-end gap-2">
                              <button type="button" onClick={() => setViewContribRow(row)} className="rounded-md border border-gray-200 bg-white px-3 py-1 font-semibold text-gray-700 hover:bg-gray-50 text-xs">View</button>
          ...
          <div>
            {expenseLoading ? <div className="p-5 flex items-center justify-center"><Spinner className="text-gray-400" /></div> : null}
            {expenseError ? (
              <div className="p-5">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {expenseRows.map((row, idx) => (
                        <tr key={row?._id ?? `e-${idx}`} className="text-sm text-gray-700">
                          <td className="px-6 py-1.5 text-gray-900" title={row?.spentOn || ""}>
                            <span className="sm:hidden">{truncateMobileName(row?.spentOn)}</span>
                            <span className="hidden sm:inline">{truncateDesktopName(row?.spentOn)}</span>
                          </td>
      ...
      {viewContribRow ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 overflow-y-auto" onClick={() => setViewContribRow(null)}>
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
              ...
              <button type="button" onClick={() => setViewContribRow(null)} className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-700 hover:bg-gray-50" aria-label="Close">
            ...
            <div className="px-5 py-4 space-y-3 text-sm">
              <div>
                <div className="text-xs font-semibold text-gray-500">Contributor</div>
                <div className="mt-1 text-gray-900">{viewContribRow?.contributorName || "—"}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-gray-500">Amount</div>
                <div className="mt-1 text-gray-900">{formatCurrency(viewContribRow?.amount || 0, currency)}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-gray-500">Date</div>
                <div className="mt-1 text-gray-900">{formatDate(viewContribRow?.date)}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-gray-500">Notes</div>
                <div className="mt-1 text-gray-900 whitespace-pre-wrap">{viewContribRow?.notes || "—"}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-gray-500">Recorded By</div>
                <div className="mt-1 text-gray-900">{viewContribRow?.createdBy?.fullName || "—"}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {viewExpenseRow ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 overflow-y-auto" onClick={() => setViewExpenseRow(null)}>
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-5 py-4 space-y-3 text-sm">
              <div>
                <div className="text-xs font-semibold text-gray-500">Spent On</div>
                <div className="mt-1 text-gray-900">{viewExpenseRow?.spentOn || "—"}</div>
              </div>
              <div>
                <div className="text-xs font-semibold text-gray-500">Amount</div>
              ...
              <div>
                <div className="text-xs font-semibold text-gray-500">Recorded By</div>
                <div className="mt-1 text-gray-900">{viewExpenseRow?.createdBy?.fullName || "—"}</div>
              </div>
            </div>
          </div>
        </div>
