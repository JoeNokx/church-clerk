          ...
          <tbody className="divide-y divide-gray-200">
            {rows.map((row, index) => (
              <tr key={row?._id ?? `row-${index}`} className="text-sm text-gray-700">
                <td className="px-6 py-1.5 text-gray-900" title={row?.beneficiaryName || ""}>
                  <span className="sm:hidden">{truncateMobileName(row?.beneficiaryName)}</span>
                  <span className="hidden sm:inline">{truncateDesktopName(row?.beneficiaryName)}</span>
                </td>
          ...
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-5 py-4 space-y-3 text-sm">
              <div>
                ...
                <div className="mt-1 text-gray-900">{viewRow?.beneficiaryName || "—"}</div>
              </div>
              <div>
