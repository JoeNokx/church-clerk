...
function EventCreatePage({ open, onClose, onSuccess, mode = "create", eventId: eventIdProp }) {
  ...
  const eventId = eventIdProp || (isEdit ? params.get("id") : null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setTimeFrom(String(e?.timeFrom || "").trim());
        setTimeTo(String(e?.timeTo || "").trim());
  ...
  }, [isModal, open, isEdit, eventId, resetForm]);
  ...
  const goBack = useCallback(() => {
    ...
    navigate(-1);
  }, [eventId, isModal, location?.state?.from, navigate, onClose, toPage]);
        ...
        <form onSubmit={onSubmit} className={isModal ? "flex flex-col min-h-0" : "mt-6 rounded-xl border border-gray-200 bg-white"}>
          {isModal ? null : (
            <div className="border-b border-gray-200 px-5 py-4">
              ...
              <div className="mt-1 text-xs text-gray-500">Fill the details below</div>
            </div>
          )}
