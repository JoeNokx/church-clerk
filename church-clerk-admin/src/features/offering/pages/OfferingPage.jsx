...
export function OfferingPageInner({ hideKpi = false, embedded = false }) {
  ...
  const canRead = useMemo(() => (typeof can === "function" ? can("offerings", "read") : false), [can]);
  ...
  }, [store?.activeChurch, canRead]);

  useEffect(() => {
    if (hideKpi) return;
    if (!canRead) return;
    refreshKpi();
  }, [refreshKpi, hideKpi, canRead]);
  ...
  if (!canRead) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5 text-sm text-gray-700">
        You do not have permission to view offerings.
      </div>
    );
  }
