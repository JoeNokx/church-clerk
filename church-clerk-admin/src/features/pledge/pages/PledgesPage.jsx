...
function PledgeFormModal({ open, mode, initialData, onClose, onSubmit, currency, title }) {
  ...
  return (
    <BaseModal
      open={open}
      title={title || (mode === "edit" ? "Edit Pledge" : "New Pledge")}
  ...
  if (!canRead) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5 text-sm text-gray-700">
        You do not have permission to view pledges.
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-purple-50 text-purple-700">
              <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5">
                ...
                <path
                  d="M17 5.5c0-2-2.24-3.5-5-3.5s-5 1.5-5 3.5 2.24 3.5 5 3.5 5 1.5 5 3.5-2.24 3.5-5 3.5-5-1.5-5-3.5"
                  stroke="currentColor"
                  strokeWidth="1.8"
                  strokeLinecap="round"
                />
                ...
                <tbody className="divide-y divide-gray-200">
                  {rows.map((row, index) => (
                    <tr key={row?._id ?? `row-${index}`} className="text-sm text-gray-700">
                      ...
                      <td className="px-6 py-1.5 text-red-600 whitespace-nowrap">
