...
function BusinessVenturesPage() {
  ...
  const load = async () => {
    ...
    const [venturesRes, kpiRes] = await Promise.allSettled([getBusinessVentures({ page: 1, limit: 50 }), getBusinessKPI()]);
    ...
    if (venturesRes.status !== "fulfilled") {
      setError(
        venturesRes.reason?.response?.data?.message || venturesRes.reason?.message || "Failed to load business ventures"
      );
    }
  ...
  if (!canRead) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5 text-sm text-gray-700">
        You do not have permission to view business ventures.
      </div>
    );
  }
