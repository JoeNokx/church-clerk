...
import {
  ...
  updateGroupOffering,
  deleteGroupOffering
} from "../../group/offering/services/groupOffering.api.js";
...
function OrganisationDetailsPage() {
  ...
  const loadAttendances = useCallback(async () => {
    ...
    try {
      if (type === "department") {
        ...
        const res = await getMinistryAttendances(id);
        const payload = res?.data?.data ?? res?.data;
        setAttendances(Array.isArray(payload?.attendances) ? payload.attendances : []);
      } else {
  ...
  const loadOfferings = useCallback(async () => {
    ...
    try {
      if (type === "department") {
        ...
        const res = await getMinistryOfferings(id);
        const payload = res?.data?.data ?? res?.data;
        setOfferings(Array.isArray(payload?.offerings) ? payload.offerings : []);
      } else {
  ...
  const titleColor = type === "group" ? "bg-blue-100 text-blue-700" : type === "cell" ? "bg-orange-100 text-orange-700" : type === "ministry" ? "bg-pink-100 text-pink-700" : "bg-purple-100 text-purple-700";

  return (
    <div className="max-w-6xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <button type="button" onClick={goBack} className="inline-flex items-center gap-2 text-sm font-semibold text-gray-700 hover:underline">
            ...
            <div className="flex items-start gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${titleColor}`}>{title}</span>
      ...
      {activeTab === "members" ? (
        <div className="mt-6 rounded-xl border border-gray-200 bg-white">
          ...
          <SimpleModal open={addMemberOpen} title="Add Member" onClose={() => setAddMemberOpen(false)}>
      ...
      {activeTab === "attendance" ? (
        <div className="mt-6 rounded-xl border border-gray-200 bg-white">
          ...
          <SimpleModal open={attendanceOpen} title={attendanceMode === "edit" ? "Edit Attendance" : "Add Attendance"} onClose={() => setAttendanceOpen(false)}>
      ...
      {activeTab === "offerings" ? (
        <div className="mt-6 rounded-xl border border-gray-200 bg-white">
          ...
          <SimpleModal open={offeringOpen} title={offeringMode === "edit" ? "Edit Offering" : "Add Offering"} onClose={() => setOfferingOpen(false)}>
